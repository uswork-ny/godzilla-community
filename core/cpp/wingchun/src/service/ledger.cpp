        ...
        namespace service
        {
            ...
            void Ledger::on_start()
            {
                ...
                $([&](event_ptr event)
                  {
                      try
                      { on_depth(event, event->data<Depth>()); }
                ...
                $([&](event_ptr event)
                  {
                      try
                      { on_transaction(event, event->data<MyTrade>()); }
                      catch (const std::exception &e)
                      {
                          SPDLOG_ERROR("Unexpected exception {}", e.what());
                      }
            ...
            {
                events_ | from(md_location_uid) | is(msg::type::Depth) | first() |
            ...
            {
                events_ | from(md_location_uid) | is(msg::type::Depth) | timeout(std::chrono::seconds(5)) |
                $([&](event_ptr e)
                  {},
            ...
            void Ledger::request_subscribe(uint32_t account_location_id, const std::vector<msg::data::Instrument> &instruments)
            {
                ...
                auto write_sub_msg = [=]()
                {
                    //TODO, subscribe in ledger
                    // for (const auto &instrument : instruments)
                    // {
                    //     write_subscribe_msg(this->get_writer(md_location->uid), now(), instrument.exchange_id, instrument.symbol, "depth");
                    // }
                };
