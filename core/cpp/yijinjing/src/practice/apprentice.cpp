    ...
    namespace practice
    {
        ...
        void apprentice::on_write_to(const yijinjing::event_ptr &event)
        {
            ...
            if (writers_.find(request.dest_id) == writers_.end())
            {
                ...
                SPDLOG_TRACE("writer from [{:08x}] to [{:08x}] ", event->source(), request.dest_id);
            } else
            {
        ...
        void apprentice::on_read_from(const yijinjing::event_ptr &event)
        {
            ...
            SPDLOG_TRACE("{} [{:08x}] asks observe at {} [{:08x}] {} from {}", get_location(event->source())->uname, event->source(),
