...
struct api {
    ...
    api(
        ...
        ,std::string sk
        ,std::size_t timeout
        ,std::string client_api_string = "Godzilla-Binance-0.0.1"
    );
    virtual ~api();
    ...
    result<order_info_t>
    future_order_info(const std::string &symbol, std::size_t orderid, const std::string &client_orderid = std::string{}, order_info_cb cb = {})
    { return future_order_info(symbol.c_str(), orderid, client_orderid.empty() ? nullptr : client_orderid.c_str(), std::move(cb)); }
    result<order_info_t>
    future_order_info(const char *symbol, std::size_t orderid, const char *client_orderid = nullptr, order_info_cb cb = {});
    ...
    result<cancel_order_info_t>
    future_cancel_order(const char *symbol, std::size_t order_id, const char *client_order_id, cancel_order_cb cb = {});
    ...
    using future_exchange_info_cb = std::function<bool(const char *fl, int ec, std::string errmsg, future_exchange_info_t res)>;
    result<future_exchange_info_t>
    future_exchange_info(future_exchange_info_cb cb = {});

    result<start_user_data_stream_t>
    future_start_user_data_stream(start_user_data_stream_cb cb = {});

    result<ping_user_data_stream_t>
    future_ping_user_data_stream(const std::string &listen_key, ping_user_data_stream_cb cb = {}) { return future_ping_user_data_stream(listen_key.c_str(), std::move(cb)); }
    result<ping_user_data_stream_t>
    future_ping_user_data_stream(const char *listen_key, ping_user_data_stream_cb cb = {});

    using future_new_order_cb = std::function<bool(const char *fl, int ec, std::string errmsg, future_new_order_resp_t res)>;
    result<future_new_order_resp_t>
    future_new_order(
         const std::string &symbol
        ,e_side side
        ,e_position_side positionSide
        ,e_type type
        ,const std::string reduceOnly
        ,const std::string &price
        ,const std::string &quantity
        ,const std::string &newClientOrderId
        ,const std::string &stopPrice
        ,const std::string &closePosition
        ,const std::string &activationPrice
        ,const std::string &callbackRate
        ,e_time timeInForce
        ,const std::string &workingType
        ,const std::string &priceProtect
        ,e_trade_resp_type newOrderRespType
        ,std::size_t recvWindow
        ,future_new_order_cb cb = {}
    ) {
        return future_new_order(
            symbol.c_str()
            ,side
            ,positionSide
            ,type
            ...
            ,price.c_str()
            ,quantity.c_str()
            ,newClientOrderId.c_str()
            ,stopPrice.c_str()
            ,closePosition.c_str()
            ,activationPrice.c_str()
            ,callbackRate.c_str()
            ,timeInForce
            ,workingType.c_str()
            ,priceProtect.c_str()
    ...
    future_new_order(
        ...
        ,const char *stopPrice
        ,const char *closePosition
        ,const char *activationPrice
        ,const char *callbackRate
        ,e_time timeInForce
        ,const char *workingType
        ,const char *priceProtect
        ,e_trade_resp_type newOrderRespType
        ,std::size_t recvWindow
        ,future_new_order_cb cb = {}
    );

    using future_adjust_leverage_cb = std::function<bool(const char *fl, int ec, std::string errmsg, leverage_info_t res)>;
    result<leverage_info_t> ffuture_adjust_leverage(
        const std::string &symbol,
        const int leverage,
        future_adjust_leverage_cb cb = {});
