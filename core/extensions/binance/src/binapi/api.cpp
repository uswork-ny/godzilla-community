...
struct api::impl {
    ...
    post(bool _signed, const char *target, boost::beast::http::verb action, const std::initializer_list<kv_type> &map, CB cb) {
	    ...
	    SPDLOG_TRACE("m_host={}, m_port={}, starget={}", m_host, m_port, starget);
        std::string data;
    ...
    sync_post(const char *target, boost::beast::http::verb action, std::string data) {
        ...
        oss << req;
        SPDLOG_TRACE("sync_post(): target={}, REQUEST:\n{}", target, oss.str());
    ...
    void async_post() {
        ...
        std::stringstream oss;
        oss << *req;
        SPDLOG_TRACE("async_post(): target={}, REQUEST:\n{}", target, oss.str());
    ...
    void process_reply(const char *fl, int ec, std::string errmsg, std::string body) {
        ...
        __TRY_BLOCK() {
            ...
            SPDLOG_TRACE("process_reply(): target={}, recv buf={}", item.target, body);
...
api::result<order_info_t> api::future_order_info(const char *symbol, std::size_t orderid, const char *client_orderid, order_info_cb cb) {
    const impl::init_list_type map = {
         {"symbol", symbol}
        ,{"orderId", orderid}
        ,{"origClientOrderId", client_orderid}
...
api::result<cancel_order_info_t> api::future_cancel_order(
     const char *symbol
    ,std::size_t order_id
    ,const char *client_order_id
    ,cancel_order_cb cb
...
/*************************************************************************************************/
api::result<future_new_order_resp_t>
api::future_new_order(
     const char *symbol
    ,const e_side side
    ,const e_position_side positionSide
    ,const e_type type
    ...
    ,const e_time timeInForce
    ,const char *workingType
    ,const char *priceProtect
    ,const e_trade_resp_type newOrderRespType
    ,const std::size_t recvWindow
    ,future_new_order_cb cb
) {
    //TODO, only support market/limit order.
    if (type == binapi::e_type::limit) {
        const impl::init_list_type map = {
            {"symbol", symbol}
            ,{"side", e_side_to_string(side)}
            ,{"type", e_type_to_string(type)}
            ,{"positionSide", e_position_side_to_string(positionSide)}
            ,{"price", price}
            ,{"quantity", quantity}
            ,{"newClientOrderId", newClientOrderId}
            ,{"timeInForce", e_time_to_string(timeInForce)}
        };
        return pimpl->post(true, "/fapi/v1/order", boost::beast::http::verb::post, map, std::move(cb));
    } else if (type == binapi::e_type::market) {
        const impl::init_list_type map = {
            {"symbol", symbol}
            ,{"side", e_side_to_string(side)}
            ,{"type", e_type_to_string(type)}
        ...
        const impl::init_list_type map = {};
        return pimpl->post(true, "/fapi/v1/order", boost::beast::http::verb::put, map, std::move(cb));
    }
}

api::result<leverage_info_t> api::ffuture_adjust_leverage(
    ...
    return pimpl->post(true, "/fapi/v1/leverage", boost::beast::http::verb::post, map, std::move(cb));
}

api::result<start_user_data_stream_t> api::future_start_user_data_stream(start_user_data_stream_cb cb) {
    return pimpl->post(false, "/fapi/v1/listenKey", boost::beast::http::verb::post, {}, std::move(cb));
}

api::result<ping_user_data_stream_t> api::future_ping_user_data_stream(const char *listen_key, ping_user_data_stream_cb cb) {
    const impl::init_list_type map = {
        {"listenKey", listen_key}
    };
...
api::result<future_exchange_info_t> api::future_exchange_info(future_exchange_info_cb cb) {
