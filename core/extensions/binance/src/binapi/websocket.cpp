...
struct websocket: std::enable_shared_from_this<websocket> {
    ...
    explicit websocket(boost::asio::io_context &ioctx)
        ...
        ,m_timer(ioctx, 5s)
    {
        // to prevent websocket session expiration without being notified
        // we generate a random reconnecting time between 3 and 4 hours
        // then we check if the time is reached and disconnect the
        // ws connection.
        start_time_ = std::chrono::steady_clock::now();
        std::random_device rd;
        std::mt19937 gen(rd());
        std::uniform_int_distribution<int> dist(10800, 14400);
        int rand_duration = dist(gen);
        SPDLOG_DEBUG("will restart in {} seconds", rand_duration);
        m_handler = [this, rand_duration](const boost::system::error_code &ec) {
            if (ec) {
                return;
            }
            auto end = std::chrono::steady_clock::now();
            auto duration = end - start_time_;
            if (std::chrono::duration_cast<std::chrono::seconds>(duration).count() > rand_duration) {
                stop();
            } else {
                m_timer.expires_at(m_timer.expires_at() + 5s);
                m_timer.async_wait(m_handler);
            }
        };
        m_timer.async_wait(m_handler);
    }
...
private:
    void async_start(const std::string &host, const std::string &port, const std::string &target, on_message_received_cb cb, holder_type holder) {
        m_host = host;
        m_target = target;
        SPDLOG_DEBUG("async_start, m_host: {}, m_target: {}", m_host, m_target);
    ...
    void on_read(boost::system::error_code ec, std::size_t rd, on_message_received_cb cb, holder_type holder) {
        ...
        SPDLOG_TRACE("websocket recv buf: {}", strbuf);
...
struct websockets::impl {
    ...
    websockets::handle start_channel(const char *pair, const char *channel, F cb) {
        ...
        static const auto deleter = [this](websocket *ws) {
            SPDLOG_WARN("delete websocket: {}", fmt::ptr(ws));
        ...
        auto *ptr = ws.get();
	    SPDLOG_TRACE("m_host: {}, m_port: {}, schannel: {}", m_host, m_port, schannel);
        ptr->start(
             m_host
            ,m_port
            ,schannel
        ...
        SPDLOG_DEBUG("handle in WS: {}", fmt::ptr(ptr));
        return ptr;
    }

    template<typename F>
    void stop_channel_impl(handle h, F f) {
        auto it = m_set.find(static_cast<websocket *>(h));
    ...
    bool is_ready(websocket *h) {
        auto it = m_set.find(h);
        if (it == m_set.end() or (*it)->m_stop_requested) {
            return false;
        }
        return true;
    }
...
websockets::handle websockets::future_part_depth(const char *pair, e_levels level, e_freq freq, on_part_depths_received_cb cb) {
    std::string ch = "depth";
    ch += std::to_string(static_cast<std::size_t>(level));
    ch += "@";
    ch += std::to_string(static_cast<std::size_t>(freq)) + "ms";
...
/*************************************************************************************************/
websockets::handle websockets::mark_price(const char *pair, on_mark_price_received_cb cb)
{ return pimpl->start_channel(pair, "markPrice", std::move(cb)); }
        ...
        (const char *fl, int ec, std::string errmsg, userdata::userdata_stream_t msg)
    {
        ...
        switch ( ehash ) {
            ...
            case fnv1a("listStatus"): {
                SPDLOG_DEBUG("[listStatus] not implemented");
                return true;
            }
            default: {
                assert(!"unreachable");
...
websockets::handle websockets::future_order_data(
         const char *lkey
        ,future_order_update_cb order_update
    ) {
    auto cb = [ocb=std::move(order_update)]
        (const char *fl, int ec, std::string errmsg, userdata::userdata_stream_t msg)
    {
        if ( ec ) {
            ocb(fl, ec, std::move(errmsg), userdata::future_order_update_t{});
            return false;
        }
        ...
        switch ( ehash ) {
            ...
            case fnv1a("ORDER_TRADE_UPDATE"): {
                userdata::future_order_update_t res = userdata::future_order_update_t::construct(json);
                return ocb(fl, ec, std::move(errmsg), std::move(res));
            }
            case fnv1a("ACCOUNT_CONFIG_UPDATE"): {
                SPDLOG_DEBUG("ACCOUNT_CONFIG_UPDATE not implemented");
                return true;
            }
            default: {
                SPDLOG_DEBUG("unreachable");
                return true;
            }
        }
...
bool websockets::fetch_reconnect_flag() {
    auto flag = pimpl->m_reconnected or pimpl->m_set.size() == 0;
    pimpl->m_reconnected = false;
    return flag;
}
bool websockets::is_ready(websockets::handle h) {
    return pimpl->is_ready((websocket *)h);
}
